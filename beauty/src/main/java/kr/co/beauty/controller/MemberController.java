...
//		log.info("vo : " + vo);
	...
	public String find(Model model, Principal principal, @CookieValue(required = false) String nomember,
			HttpSession session) {
		// 장바구니 카운터
		String cartCount = (String) session.getAttribute("cartCount");
		if (cartCount == null) {
	...
	public String findPwResult(Model model, Principal principal, @CookieValue(required = false) String nomember,
			HttpSession session) {
		String uid = (String) session.getAttribute("rs");
		model.addAttribute("uid", uid);
		// 장바구니 카운터
