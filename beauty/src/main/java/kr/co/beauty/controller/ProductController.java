...
public class ProductController {
    ...
    // 상품리스트 출력
    @GetMapping("shop/list")public String productList(Model model, int cate, Principal principal, @CookieValue(required = false)String nomember, @RequestParam(required = false)String sort, @RequestParam(required = false)Integer pg, HttpSession session) {
        if (pg == null) {
            pg = 1;
        }
        int start = (pg - 1) * 20;
    ...
    // 상품보기
    @GetMapping("shop/view")public String productView(Model model, @RequestParam("pno")String prodNo, Principal principal, @CookieValue(required = false)String nomember, HttpServletResponse resp, HttpSession session) {
    ...
    // 위시리스트
    @PostMapping("addWish")@ResponseBody public Map<String, Integer> wish(WishVO vo) {
        Map < String,
    ...
    // 장바구니 등록
    @PostMapping("addCart")@ResponseBody public Map<String, Integer> cart(@RequestParam Map data, Principal principal, @CookieValue(required = false)String nomember, HttpServletResponse resp, HttpSession session)throws Exception {
        String uid = null;
    ...
    // 주문하기
    @PostMapping("addOrder")@ResponseBody public Map<String, Integer> order(@RequestParam Map data, Principal principal, @CookieValue(required = false)String nomember, HttpSession session, HttpServletResponse resp)throws Exception {
        String uid = null;
        if (principal != null) { // 회원ㅇ 로그인 상태
            uid = principal.getName();
        } else if (nomember == null) { // 쿠키가 없을 때(새 쿠키생성 2일짜리)
